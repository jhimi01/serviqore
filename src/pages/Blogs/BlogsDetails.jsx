    ...
    return (
        <div className='my-24 md:w-9/12 w-11/12 mx-auto'>
            ...
            <div className='w-full'>
                <img className='w-full h-full rounded hover:shadow-md' src={blogPost.image} alt={blogPost.title} />
            </div>
            <div>
                <p className='text-black font-semibold text-xl mt-5'>By {blogPost.author} | {blogPost.date}</p>
                <p className='text-textMuted my-3'>{blogPost.description}</p>
                <div>
                    {blogPost.tags.map((tag, index) => (
                        <span  className="bg-gray-200 text-primary text-xs font-semibold rounded-full px-2 py-1 mr-2 mb-2" key={index}>{tag}</span>
                    ))}
                </div>
            </div>
        </div>
